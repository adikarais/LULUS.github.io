...
// Mendapatkan filter pelajaran dan lokasi dari parameter GET
...
// Menambahkan kondisi filter pelajaran jika ada
if ($filter_pelajaran > 0) {
   $conditions[] = "pj.pelajaran_id = " . $filter_pelajaran;
}

// Menambahkan kondisi filter lokasi jika ada
if ($filter_lokasi > 0) {
   $conditions[] = "pj.lokasi_id = " . $filter_lokasi;
}

// Membuat klausa WHERE berdasarkan kondisi yang ada
$where = "";
if (!empty($conditions)) {
   $where = "WHERE " . implode(" AND ", $conditions);
}

// Query untuk mendapatkan data lokasi dan pelajaran berdasarkan filter
$query = "
...
<style>
   ...
   #filterForm, #filterForm label, #filterForm select, #filterForm .form-check-input {
     font-size: 1.5rem;
   }
...
<script>
   ...
   // Fungsi untuk mengirimkan form dengan kategori yang dipilih
